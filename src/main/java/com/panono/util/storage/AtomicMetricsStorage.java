...
public class AtomicMetricsStorage<T> implements MetricsStorage<T> {
	...
	public AtomicMetricsStorage(TemporalUnit targetUnit, TemporalUnit groupingUnit, int bufferSize,
			Supplier<T> factory) {
		this(targetUnit, groupingUnit, bufferSize, factory, System::currentTimeMillis);
	}

	@VisibleForTesting
	protected AtomicMetricsStorage(TemporalUnit targetUnit, TemporalUnit groupingUnit, int bufferSize,
			Supplier<T> factory, Supplier<Long> now) {
		this.now = now;
	...
	private Stream<T> getReferenceStream() {
		long nowLong = this.now.get();

		int firstIndex = minimalIndexFor(nowLong);
		int lastIndex = currentIndexFor(nowLong);

		return IntStream.rangeClosed(firstIndex, lastIndex)
				.mapToObj(index -> historical(index, store.get(offset(index)))).filter(Objects::nonNull)
	...
	private int checkedIndexFor(long timestamp) {
		...
		int minimalIndex = minimalIndexFor(nowLong);
		int maximalIndex = currentIndexFor(nowLong);
